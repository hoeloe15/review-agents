"""
Utility functions for the Review Writer Agent system.

This module provides helper functions for setting up and running the review writer
agent system, including kernel initialization and plugin registration.
"""
...
from semantic_kernel.core_plugins import TimePlugin, MathPlugin, TextPlugin
...
def setup_kernel_with_plugins() -> Kernel:
    """
    Set up a kernel with all necessary plugins for the review system.
    
    Returns:
        A Kernel instance with all required plugins registered.
    """
    # Get a kernel instance from the provider
    kernel = KernelProvider.get_kernel()
    ...
    # Register custom review plugins
    kernel.add_plugin(ReviewPlugin(), "review")
    
    logger.info("Kernel set up with all required plugins")
    return kernel
